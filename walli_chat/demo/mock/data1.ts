...
const BASE_MESSAGE_SPECS: MarkdownChatSeed[] = [
  ...
  message("assistant", '![mountain lake](/demo-landscape-lake.jpg){width="480" height="320"}'),
...
export function getMessages() {
  ...
  messages.push(
    ...
    },
    {
      ...
      markdown: [
        "# 项目交付说明",
        "这是一条 **people 复杂 Markdown** 消息，包含 *强调*、~~删除线~~、`inline code` 和 [文档链接](https://www.typescriptlang.org/docs/)。",
        "",
        "## 验收清单",
        "- 检查头像、昵称与消息气泡",
        "  - 宽屏下保持内容宽度",
        ...
        "3. 记录验收结果",
        "",
        "> **注意**：正文沿用 Markdown 渲染，头像、昵称和气泡由 people 外壳提供。",
        ">",
        "> 这段引用用于检查多行换行、左侧引用线与段落间距。",
        "",
        "## 代码示例",
        "```ts",
        "type Person = { nickname: string; avatarUrl?: string };",
        "",
        "function greet(person: Person): string {",
        "  return `Hello, ${person.nickname}!`;",
        "}",
        "",
        'console.log(greet({ nickname: "Alex" }));',
        "```",
        "",
        "## 测试结果",
        "| 功能 | 状态 | 说明 |",
        "| :--- | :---: | ---: |",
        "| 普通文本 | ✅ | 短句按内容收缩 |",
        "| 长段落 | ✅ | 在容器变窄时自动换行，不覆盖头像或昵称 |",
        "| 代码与表格 | 待验收 | 检查边界、背景和横向滚动 |",
        "",
        "---",
        "",
        "### 图片与提示卡片",
        "![海岸风景](/demo-landscape-coast.jpg)",
        "",
        createNoticeMarkdown({
          text: "这是嵌套在 people 消息中的自定义 Notice block。",
          variant: "info",
        }),
        "",
